...
# TODO: Drop the old commit column
...
for project in config.sections():
    ...
    # Retrieve the repo.repo_id value for the project
    sql = 'SELECT repo_id FROM repo WHERE name = :repo_name'
    c.execute(sql, {"repo_name": project})
    result = c.fetchall()
    if len(result) > 0:
        repo_id = result[0][0]

        # Update old social_stats.project column data, to refer to repo.repo_id instead
        # (this is converting from an old schema layout, to the current one)
        try:
            sql = 'UPDATE social_stats SET project = :repo_id WHERE project = :project'
            c.execute(sql, {"repo_id": repo_id, "project": project})
            result = c.fetchall()
    ...
    c.execute(sql, {"project": repo_id, "now": now, "watchers": watchers, "stars": stars, "forks": forks,
